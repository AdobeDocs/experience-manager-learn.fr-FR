...
1. A **Auteur de contenu** utilise le service de création d’AEM pour créer, modifier et gérer du contenu.
...
3. Une fois que le contenu a été validé, il peut être **publié** au service AEM Publish.
...
Le tutoriel simule le déploiement ci-dessus en ajoutant une instance AEM Publish à la configuration actuelle. Dans les chapitres précédents, React App a servi de prévisualisation en se connectant directement à l’instance d’auteur. Une version de production de l’application React est déployée sur un serveur Node.js statique qui se connecte à la nouvelle instance de publication.

En fin de compte, trois serveurs locaux sont en cours d’exécution :
...
Actuellement, nous disposons d’une instance en cours d’exécution du SDK dans **Auteur** mode . Le SDK peut également être démarré dans **Publier** pour simuler un environnement de publication AEM.

Un guide plus détaillé pour la configuration d’un environnement de développement local [peut être consulté ici](https://experienceleague.adobe.com/docs/experience-manager-learn/cloud-service/local-development-environment-set-up/overview.html?lang=fr#local-development-environment-set-up).

1. Sur votre système de fichiers local, créez un dossier dédié à l’installation de l’instance de publication, c’est-à-dire nommé `~/aem-sdk/publish`.
1. Copiez le fichier JAR Quickstart utilisé pour l’instance d’auteur dans les chapitres précédents et collez-le dans le `publish` répertoire . Vous pouvez également accéder au [Portail de distribution de logiciels](https://experience.adobe.com/#/downloads/content/software-distribution/en/aemcloud.html) et téléchargez le dernier SDK et extrayez le fichier JAR de démarrage rapide.
1. Renommez le fichier jar en `aem-publish-p4503.jar`.

   Le `publish` string indique que le fichier Quickstart jar démarre en mode de publication. Le `p4503` indique que le serveur Quickstart s’exécute sur le port 4503.
...
1. Indiquez un mot de passe administrateur en tant que `admin`. Tout mot de passe administrateur est acceptable, mais il est recommandé d’utiliser la valeur par défaut pour le développement local afin d’éviter des configurations supplémentaires.
...
1. Connectez-vous à l’instance de publication en accédant directement à : [http://localhost:4503/libs/granite/core/content/login.html](http://localhost:4503/libs/granite/core/content/login.html) avec le nom d’utilisateur `admin` et mot de passe `admin`.
1. Ensuite, accédez à Package Manager à l’adresse [http://localhost:4503/crx/packmgr/index.jsp](http://localhost:4503/crx/packmgr/index.jsp).
...
1. Se déconnecter en tant que `admin` en cliquant sur le bouton &quot;Se déconnecter&quot; dans la barre de menus.
...
Mettez ensuite à jour les variables d’environnement utilisées par l’application React pour pointer vers l’instance de publication. L’application React doit **only** connectez-vous à l&#39;instance de publication en mode de production.
...
1. Remplissez `.env.production.local` avec les éléments suivants :
   ...
   L’utilisation de variables d’environnement facilite le basculement du point d’entrée GraphQL entre un environnement de création ou de publication sans ajouter de logique supplémentaire dans le code de l’application. Plus d’informations sur [Les variables d’environnement personnalisées pour React se trouvent ici](https://create-react-app.dev/docs/adding-custom-environment-variables).
...
L’application React peut être lancée à l’aide du serveur webpack, mais cela est réservé au développement. Simulez ensuite un déploiement en production à l’aide de la fonction [serve](https://github.com/vercel/serve) pour héberger une version de production de l’application React à l’aide de Node.js.

1. Ouvrez une nouvelle fenêtre de terminal et accédez au `aem-guides-wknd-graphql/react-app` directory

   ```shell
   $ cd aem-guides-wknd-graphql/react-app
   ```

1. Installer [serve](https://github.com/vercel/serve) avec la commande suivante :

   ```shell
   $ npm install serve --save-dev
   ```

1. Ouvrez le fichier `package.json` dans `react-app/package.json`. Ajouter un script nommé `serve`:
   ...
   Le `serve` exécute deux actions. Tout d’abord, une version de production de l’application React est générée. Deuxièmement, le serveur Node.js démarre et utilise la version de production.
...
1. Ouvrez un nouveau navigateur et accédez à [http://localhost:5000/](http://localhost:5000/). Vous devriez voir l’application React diffusée.
   ...
   Notez que la requête GraphQL fonctionne sur la page d’accueil. Inspect **XHR** demandez à l’aide de vos outils de développement. Vérifiez que le POST GraphQL se trouve sur l’instance de publication à l’adresse `http://localhost:4503/content/graphql/global/endpoint.json`.
...
1. Cliquez sur l’une des pages Détails de l’aventure.
   ...
   Vérifiez qu’une erreur GraphQL est générée pour `adventureContributor`. Dans les exercices suivants, les images brisées et la `adventureContributor` les problèmes sont résolus.

## Références d’image absolue {#absolute-image-references}

Les images apparaissent rompues car la variable `<img src` est défini sur un chemin relatif et pointe vers le serveur statique Node à `http://localhost:5000/`. À la place, ces images doivent pointer vers l’instance de publication AEM. Il existe plusieurs solutions possibles. Lors de l’utilisation du serveur de développement webpack, le fichier `react-app/src/setupProxy.js` configurer un proxy entre le serveur webpack et l’instance d’auteur AEM pour toutes les demandes à `/content`. Une configuration proxy peut être utilisée dans un environnement de production, mais doit être configurée au niveau du serveur web. Par exemple : [Module proxy d’Apache](https://httpd.apache.org/docs/2.4/mod/mod_proxy.html).

L’application peut être mise à jour afin d’inclure une URL absolue à l’aide de la variable `REACT_APP_HOST_URI` Variable d’environnement. Utilisez plutôt une fonctionnalité de l’API GraphQL AEM pour demander une URL absolue à l’image.

1. Arrêtez le serveur Node.js.
1. Revenez à l’IDE et ouvrez le fichier . `Adventures.js` at `react-app/src/components/Adventures.js`.
1. Ajoutez la variable `_publishUrl` à la propriété `ImageRef` dans le `allAdventuresQuery`:
   ...
   `_publishUrl` et `_authorUrl` sont des valeurs intégrées à la variable `ImageRef` pour faciliter l’inclusion d’URL absolues.

1. Répétez les étapes ci-dessus pour modifier la requête utilisée dans le `filterQuery(activity)` pour inclure la fonction `_publishUrl` .
1. Modifiez le `AdventureItem` component at `function AdventureItem(props)` pour référencer la variable `_publishUrl` au lieu de `_path` lors de la création de la propriété `<img src=''>` tag :
...
1. Répétez les mêmes étapes pour modifier la requête GraphQL et ajouter le `_publishUrl` propriété de l’aventure
...
1. Modifiez les deux `<img>` balises pour l’image Principal aventure et la référence d’image du contributeur dans `AdventureDetail.js`:
...
1. Accédez à [http://localhost:5000/](http://localhost:5000/) et observez que les images apparaissent et que la fonction `<img src''>` pointe vers `http://localhost:4503`.
...
Rappelez-vous qu’une erreur GraphQL est générée pour `adventureContributor` lorsqu’une page Détails de l’aventure est demandée. Le **Contributeur** Le modèle de fragment de contenu n’existe pas encore sur l’instance de publication. Mises à jour apportées au **Adventure** Le modèle de fragment de contenu n’est pas non plus disponible sur l’instance de publication. Ces modifications ont été apportées directement à l’instance d’auteur et doivent être distribuées à l’instance de publication.
...
1. Téléchargez le module [EnableReplicationAgent.zip](./assets/publish-deployment/EnableReplicationAgent.zip) et installez-le à l’aide de Package Manager.

   Ce package installe une configuration qui permet à l’instance d’auteur de publier du contenu sur l’instance de publication. Étapes manuelles pour [cette configuration est disponible ici](https://experienceleague.adobe.com/docs/experience-manager-learn/cloud-service/local-development-environment-set-up/aem-runtime.html?lang=en#content-distribution).

   >[!NOTE]
   >
   > Dans un environnement as a Cloud Service AEM, le niveau Auteur est automatiquement configuré pour distribuer le contenu au niveau Publication.

1. Dans la **AEM** , accédez à **Outils** > **Ressources** > **Modèles de fragment de contenu**.

1. Cliquez dans le **Site WKND** dossier.
...
1. Sélectionnez les trois modèles et cliquez sur **Publier**:
   ...
   Une boîte de dialogue de confirmation s’affiche, puis cliquez sur **Publier**.
...
1. L’assistant de publication affiche toutes les ressources dépendantes qui doivent être publiées. Dans ce cas, le fragment référencé **stacey-roswells** est répertoriée et plusieurs images sont également référencées. Les ressources référencées sont publiées avec le fragment.

   ![Ressources référencées à publier](assets/publish-deployment/referenced-assets.png)

   Cliquez sur le bouton **Publier** pour publier à nouveau le fragment de contenu et les ressources dépendantes.
...
1. Revenez à l’instance d’auteur AEM à l’adresse [http://localhost:4502/editor.html/content/dam/wknd/en/adventures/bali-surf-camp/bali-surf-camp](http://localhost:4502/editor.html/content/dam/wknd/en/adventures/bali-surf-camp/bali-surf-camp) et mettre à jour la variable **Titre** du fragment. **Enregistrer et fermer** le fragment. Alors **publier** le fragment.
1. Revenir à [http://localhost:5000/adventure:/content/dam/wknd/en/adventures/bali-surf-camp/bali-surf-camp](http://localhost:5000/adventure:/content/dam/wknd/en/adventures/bali-surf-camp/bali-surf-camp) et observez les modifications publiées.
...
1. Revenez à la fenêtre du terminal où l’application React est en cours d’exécution avec la commande . `npm run serve`:
   ...
   Notez que deux URL sont fournies. Une utilisation `localhost` et une autre utilisant l’adresse IP du réseau local.

1. Accédez à l’adresse commençant par [http://192.168.86.XXX:5000](http://192.168.86.XXX:5000). L&#39;adresse est légèrement différente pour chaque ordinateur local. Notez qu’il existe une erreur CORS lors de la récupération des données. En effet, la configuration CORS actuelle autorise uniquement les requêtes provenant de `localhost`.
...
1. Accédez à [http://localhost:4503/content/wknd/us/en/errors/sign-in.html](http://localhost:4503/content/wknd/us/en/errors/sign-in.html) et se connecter à l’aide du nom d’utilisateur `admin` et mot de passe `admin`.
...
1. Mettez à jour le **Origines autorisées** pour inclure l’adresse IP du réseau :
...
1. Rechercher **Filtre de référent Apache Sling** et passez en revue la configuration. Le **Autoriser vide** Une configuration est également nécessaire pour activer les requêtes GraphQL provenant d’un domaine externe.

   ![Filtre référent Sling](assets/publish-deployment/sling-referrer-filter.png)

   Ils ont été configurés dans le cadre du site de référence WKND. Vous pouvez afficher l’ensemble complet des configurations OSGi via [Référentiel GitHub](https://github.com/adobe/aem-guides-wknd/tree/master/ui.config/src/main/content/jcr_root/apps/wknd/osgiconfig).

   >[!NOTE]
   >
   > Les configurations OSGi sont gérées dans un projet AEM validé dans le contrôle de code source. Un projet AEM peut être déployé dans AEM en tant qu’environnements de Cloud Service à l’aide de Cloud Manager. Le [AEM Archétype de projet](https://github.com/adobe/aem-project-archetype) peut vous aider à générer un projet pour une mise en oeuvre spécifique.

1. Revenez à l’application React en commençant par [http://192.168.86.XXX:5000](http://192.168.86.XXX:5000) et observez que l’application ne génère plus d’erreur CORS.
...
* [Déploiement du code pour AEM as a Cloud Service](https://experienceleague.adobe.com/docs/experience-manager-learn/cloud-service/cloud-manager/devops/deploy-code.html?lang=en#cloud-manager)

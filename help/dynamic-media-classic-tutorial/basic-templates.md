...
La définition d’un modèle de base (ou simplement &quot;modèle&quot;, en abrégé) est une image superposée pouvant contenir des URL. Le résultat final est une image, mais qui peut être modifiée par l’URL. Il peut se composer de photos, de texte ou d’images, quelle que soit la combinaison de ressources P-TIFF dans Dynamic Media Classic.

Les modèles sont très similaires aux fichiers PSD Photoshop, car ils disposent d’un workflow similaire et de fonctionnalités similaires.
...
6. Publiez, copiez l’URL à partir de l’aperçu de Dynamic Media Classic et testez-la dans un navigateur.
...
2. Certains types de calques et effets de calque ne sont pas pris en charge par Dynamic Media Classic ou le serveur d’images et doivent être pixellisés avant le téléchargement. Sinon, les effets peuvent être ignorés ou les calques ignorés. La pixellisation d’un calque consiste à effectuer une conversion si vous ne pouvez pas être modifié en non modifiable. Pour pixelliser les effets de calque ou les calques de texte, créez un calque vide, sélectionnez les deux calques et fusionnez à l’aide de **Calques > Fusionner les calques** ou Ctrl + E/CMD + E.
   ...
   - Les calques d’objet dynamique et les calques vectoriels seront pixellisés dans les images normales lors du téléchargement et les filtres intelligents seront appliqués et pixellisés.
...
Dynamic Media Classic prend en charge les polices OpenType standard (OTF), TrueType (TTF) et PostScript Type 1. Les polices de valise uniquement pour Mac, les fichiers de collection, les polices système Windows et les polices machine propriétaires (comme les polices utilisées par les machines de gravure ou de broderie) ne sont pas prises en charge. Vous devrez les convertir dans l’un des formats de police standard ou remplacer une police similaire à utiliser dans Dynamic Media Classic et sur le serveur d’images.

Une fois les polices chargées dans Dynamic Media Classic, comme toute autre ressource, elles doivent également être publiées sur le serveur d’images. Une erreur de modèle très courante est d’oublier de publier vos polices, ce qui entraînera une erreur d’image : le serveur d’images ne remplacera pas une autre police à sa place. En outre, si vous souhaitez utiliser l’option **Extract Text** lors du téléchargement, vous devez télécharger vos fichiers de polices avant de charger le fichier PSD qui utilise ces polices. La fonction **Extract Text** tente de recréer votre texte en tant que calque de texte modifiable et de le placer dans un modèle Dynamic Media Classic. Ceci est abordé dans la rubrique suivante, Options PSD.
...
Les formats de fichiers de polices les plus courants des systèmes Windows et Mac sont OpenType et TrueType. OpenType possède une extension .OTF, tandis que TrueType est .TTF. Les deux formats fonctionnent également bien dans Dynamic Media Classic.

### Sélection des options lors du téléchargement du fichier PSD

Vous n’avez pas besoin de charger un fichier Photoshop (PSD) pour créer un modèle ; un modèle peut être créé à partir de n’importe quelle ressource d’image dans Dynamic Media Classic. Toutefois, le téléchargement d’un fichier PSD facilite la création, car ces ressources se trouvent généralement déjà dans un fichier PSD superposé. En outre, Dynamic Media Classic génère automatiquement un modèle lorsque vous téléchargez un fichier PSD superposé.

- **Conserver les calques.** Il s’agit de l’option la plus importante. Cela indique à Dynamic Media Classic de créer une ressource image par calque Photoshop. Si cette option n’est pas cochée, toutes les autres options sont désactivées et le fichier PSD est aplati dans une seule image.
- **** **CreateTemplate.** Cette option récupère les différents calques générés et crée automatiquement un modèle en les combinant. L’inconvénient de l’utilisation du modèle généré automatiquement est que Dynamic Media Classic place tous les calques dans un fichier, alors que nous n’avons besoin que d’un seul espace réservé par calque. Il est facile de supprimer les calques supplémentaires, mais s’ils sont nombreux, il est plus rapide de les recréer. Veillez à renommer le nouveau modèle ; dans le cas contraire, il sera remplacé la prochaine fois que vous chargerez à nouveau le même fichier PSD.
- **Extraire du texte.** Les calques de texte du fichier PSD sont ainsi recréés en tant que calques de texte dans le modèle à l’aide de la police que vous avez téléchargée. Cette étape est requise si votre texte se trouve sur un chemin d’accès dans Photoshop et que vous souhaitez conserver ce chemin d’accès dans le modèle. Cette fonctionnalité nécessite l’option **Créer un modèle**, car le texte extrait ne peut être créé que par un modèle généré lors du téléchargement.
- **Etendre les calques à la taille de l’arrière-plan.** Ce paramètre donne à chaque calque la même taille que la zone de travail PSD globale. Cela s’avère très utile pour les calques qui resteront toujours en position fixe : sinon, lorsque vous permutez des images dans le même calque, vous devrez peut-être les repositionner.
- **Affectation de nom de calque.** Cela indique à Dynamic Media Classic comment nommer chaque ressource générée par calque. Nous recommandons **Photoshop** **et Layer** **Name** ou Photoshop et **Layer** **Number**. Les deux options utilisent le nom du fichier PSD comme première partie du nom et ajoutent le nom ou le numéro du calque à la fin. Par exemple, si vous disposez d’un fichier PSD nommé &quot;shirt.psd&quot; et qu’il comporte des calques nommés &quot;front&quot;, &quot;manches&quot; et &quot;colar&quot;, si vous effectuez un téléchargement à l’aide des options **Photoshop et** Calque **Nom**, Dynamic Media Classic génère les identifiants de ressources &quot;shirt_front&quot;, &quot;chemises_manches&quot; et &quot;cols&quot;.&quot; L’utilisation de l’une de ces options permet de s’assurer que le nom est unique dans Dynamic Media Classic.
...
Dans le centre gauche se trouve votre zone de travail qui affiche un aperçu de votre modèle final. Sur le côté droit se trouvent les panneaux Calques et Propriétés du calque . C&#39;est dans ces domaines que vous ferez le plus de travail.
...
3. Si aucune image n’a été sélectionnée à l’étape 1, recherchez ou accédez à une image dans le panneau Ressource à gauche et faites-la glisser sur la zone de travail.
   ...
   - L’image est automatiquement redimensionnée pour s’adapter à la taille de la zone de travail. Si vous prévoyez de permuter vos images haute résolution, vous devez généralement importer l’une de vos images P-TIFF volumineuses (2 000 pixels) et l’utiliser comme espace réservé.
...
6. Cliquez sur **Enregistrer**, sélectionnez un emplacement et donnez un nom au modèle. Vous pouvez prévisualiser, mais à ce stade, votre modèle ressemblera exactement à une image Photoshop aplatie : il n’est pas encore modifiable.
...
Enfin, votre jeu initial de paramètres devient les valeurs par défaut des propriétés dans le modèle. Si vous créez votre modèle, ajoutez des paramètres, puis appelez l’URL sans ses paramètres, le serveur d’images crée l’image avec tous les paramètres par défaut que vous avez enregistrés dans le modèle. Les paramètres ne sont nécessaires que si vous souhaitez modifier une propriété. Si une propriété n’a pas besoin d’être modifiée, il n’est pas nécessaire de définir un paramètre.
...
1. Sélectionnez l’option **On** en regard du nom de chaque propriété à transformer en paramètre. Un nom de paramètre par défaut s’affiche. Vous pouvez uniquement ajouter des paramètres aux propriétés qui ont changé de leur état par défaut.

   - Par exemple, si vous ajoutez un calque et que vous le conservez à sa position xy par défaut de 0,0, Dynamic Media Classic n’expose pas une propriété **Position**. Pour corriger, déplacez le calque d’au moins un pixel. Maintenant Dynamic Media Classic présente **Position** comme propriété que vous pouvez paramétrer.
   - Pour ajouter un paramètre à la propriété d’affichage/masquage (qui active et désactive le calque), cliquez sur l’icône **Afficher** ou **Masquer le calque** pour désactiver le calque (vous pouvez le réactiver par la suite si vous le souhaitez). Dynamic Media Classic expose désormais une propriété **Masquer** pouvant être paramétrée.
...
1. Répétez cette procédure pour les autres calques en cliquant sur le bouton **Paramètres** et en ajoutant et en renommant des paramètres.
...
La bonne nouvelle est que Dynamic Media Classic génère le RTF pour vous. Chaque fois que vous saisissez du texte dans un modèle et que vous ajoutez une mise en forme, Dynamic Media Classic écrit automatiquement le code RTF dans le modèle. La raison pour laquelle nous le mentionnons est que vous allez ajouter des paramètres directement à la RTF elle-même. Il est donc important que vous vous en rendiez un peu compte.
...
La majeure partie de votre travail se fait dans l’onglet **Aperçu**, qui vous permet de saisir du texte et de l’afficher tel qu’il apparaîtra dans le modèle. Il existe également un onglet **Source**, qui est utilisé pour modifier manuellement le RTF, si nécessaire.

Le workflow général consiste à utiliser l’onglet **Aperçu** pour saisir du texte.

Sélectionnez ensuite le texte et choisissez une mise en forme, telle que la couleur de la police, la taille de la police ou la justification, à l’aide des commandes situées en haut. Une fois que le texte est stylisé comme vous le souhaitez, cliquez sur **Appliquer** pour l’afficher dans l’aperçu de la zone de travail. Vous fermez ensuite l’éditeur de texte pour revenir à la fenêtre principale Concepts de base des modèles .
...
4. Pour mettre en forme votre texte, sélectionnez-le en faisant glisser votre souris et choisissez un contrôle de mise en forme dans l’interface utilisateur.
   ...
   - Pour choisir une autre police, veillez à sélectionner le texte et à ouvrir le menu Police . L’éditeur affiche la liste de toutes les polices chargées dans Dynamic Media Classic. Si une police est également installée sur votre ordinateur, elle s’affiche en noir. S’il n’est pas installé sur votre ordinateur, il s’affiche en rouge. Cependant, le rendu sera toujours effectué dans la fenêtre de prévisualisation lorsque vous cliquerez sur **Appliquer**. Il vous suffit de charger des polices dans Dynamic Media Classic pour les rendre disponibles à tous les utilisateurs de Dynamic Media Classic. Une fois que vous avez publié, le serveur d’images utilise ces polices pour générer le texte. Vos utilisateurs n’ont pas besoin d’installer de polices pour voir le texte que vous créez, car il fait partie d’une image.
   - Contrairement à Photoshop et Illustrator, le serveur d’images peut aligner votre texte verticalement dans la zone de texte. La valeur par défaut est l’alignement supérieur. Pour modifier ce paramètre, sélectionnez votre texte et choisissez **Milieu** ou **Bas** dans le menu **Alignement vertical** .
...
1. Cliquez sur le bouton **Paramètres** (un &quot;P&quot;) en regard du nom de la couche de texte pour laquelle vous souhaitez créer des paramètres. L’écran Paramètres s’affiche. L’onglet **Commun** répertorie chaque propriété du calque et sa valeur. Vous pouvez y ajouter des paramètres de calque standard.
1. Cliquez sur l’onglet **Texte** . Ici, vous pouvez voir le RTF en haut ; les paramètres que vous ajoutez se trouvent en dessous.
1. Pour ajouter un paramètre, mettez d’abord en surbrillance la valeur à modifier, puis cliquez sur le bouton **Ajouter un paramètre** . Veillez à ne sélectionner que les valeurs des commandes et non la commande entière. Par exemple, pour définir un paramètre pour le nom de la police dans l’exemple de RTF ci-dessus, je vais uniquement mettre en surbrillance &quot;Pauvre Richard&quot; et ajouter un paramètre à cela, mais pas également &quot;\f0&quot;. Lorsque vous cliquez sur **Ajouter un paramètre** , il apparaît dans la liste ci-dessous et la valeur du paramètre apparaît en rouge dans le RTF lorsqu’elle est toujours sélectionnée. Si vous devez supprimer un paramètre, cochez la case en regard de **On** pour désactiver ce paramètre, qui disparaîtra.
...
1. Cliquez sur **Fermer** pour quitter l’écran Paramètres. Ensuite, appuyez sur **Enregistrer** pour enregistrer le modèle. Si vous avez terminé les modifications, appuyez sur **Fermer** pour quitter la page Concepts de base des modèles .
...
_imageExemples de modèles sans paramétrage (ci-dessus) et avec des paramètres (ci-dessous)._

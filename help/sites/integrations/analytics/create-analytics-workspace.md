...
1. Configurez les variables de [conversion (eVars)](https://docs.adobe.com/content/help/en/analytics/admin/admin-tools/conversion-variables/conversion-var-admin.html) et les Événements de [réussite (Événements)](https://docs.adobe.com/help/en/analytics/admin/admin-tools/success-events/success-event.html) dans la suite de rapports.
1. Créez un projet [](https://docs.adobe.com/content/help/en/analytics/analyze/analysis-workspace/home.html) Analysis Workspace pour analyser les données à l’aide d’outils qui vous permettent de créer, d’analyser et de partager rapidement des informations.
1. Partagez le projet Analysis Workspace avec d’autres membres de l’équipe.

## Conditions préalables

Ce didacticiel est la suite du composant [Suivi sur les clics avec Adobe Analytics](./track-clicked-component.md) et suppose que vous avez :
...
1. A l’aide de la nouvelle **option** Ajouter, créons des variables de conversion pour mapper le schéma comme suit :
...
1. Dans la fenêtre Gestionnaire **de Report Suites, sélectionnez l’ID** de la suite de **rapports et cliquez sur** Modifier les paramètres ****.
1. Cliquez sur **Conversion** > Événements **de réussite**
1. A l&#39;aide de l&#39;option **Ajouter nouveau** , créez un événement de réussite personnalisé pour suivre le clic sur le bouton CTA, puis **enregistrez** vos modifications.
   * `Event` : `event8`
   * `Name`:`CTA Click`
   * `Type`:`Counter`
...
Ensuite, créez un nouveau [projet](https://docs.adobe.com/content/help/en/analytics/analyze/analysis-workspace/build-workspace-project/t-freeform-project.html) pour créer un tableau de bord afin d&#39;analyser les performances des boutons CTA sur l&#39;ensemble du site.

1. Dans la barre d’outils Analytics, sélectionnez **Workspace** et cliquez sur pour **créer un projet**.

   ![Workspace](assets/create-analytics-workspace/create-workspace.png)

1. Choisissez d&#39;effectuer un début à partir d&#39;un projet **** vierge ou de sélectionner l&#39;un des modèles prédéveloppés, fournis par Adobe ou des modèles personnalisés créés par votre organisation. Plusieurs modèles sont disponibles, selon l’analyse ou le cas d’utilisation que vous avez en tête. [En savoir plus](https://docs.adobe.com/content/help/en/analytics/analyze/analysis-workspace/build-workspace-project/starter-projects.html) sur les différentes options de modèle disponibles.
...
1. Faites glisser et déposez la mesure **CTA Click** (événement8) sur la mesure d&#39;occurrences et remplacez-la. Vous pouvez désormais vue une visualisation qui affiche le nom de page (eVar9) et un nombre correspondant de événements de clics CTA sur une page.
...
1. Vous pouvez remplacer la valeur ID de bouton par un nom plus convivial à l’aide des classifications Adobe Analytics. Vous pouvez en savoir plus sur la création d’une classification pour une mesure spécifique [ici](https://docs.adobe.com/content/help/en/analytics/components/classifications/c-classifications.html). Dans ce cas, nous avons configuré une mesure de classification `Button Section (Button ID)` pour `eVar8` laquelle l’ID de bouton est associé à un nom convivial.
...
1. Sélectionnez l’identifiant **de suite de** rapports dans la fenêtre Gestionnaire **de suites de** rapports et cliquez sur **Modifier les paramètres** > **Conversion > Classifications des conversions.******

   ![Classification des conversions](assets/create-analytics-workspace/conversion-classification.png)

1. Dans la liste déroulante **Sélectionner un type** de classification, sélectionnez la variable (ID eVar8-Bouton) pour ajouter une classification.
...
1. Téléchargeons un modèle de classification pour la variable de conversion à partir de l&#39;onglet Modèle **de** téléchargement.
...
1. Dans la barre d’outils Analytics, sélectionnez **Workspace** et ouvrez l’espace de travail que nous avons créé dans [Création d’un projet dans Analysis Workspace](#workspace-project) , dans la section de ce didacticiel.

   ![ID de bouton de l’espace de travail](assets/create-analytics-workspace/workspace-report-button-id.png)

1. Ensuite, remplacez la mesure ID **de** bouton dans votre espace de travail qui affiche l’ID d’un bouton d’appel à l’action (CTA) par le nom de classification créé à l’étape précédente.

1. Dans l’outil de recherche de composants, recherchez les boutons **CTA** WKND et faites glisser la dimension Boutons CTA **WKND (ID de bouton)** sur la mesure Id de bouton et remplacez-la.

...
Dans ce didacticiel, vous allez déclencher une règle de lancement basée sur un événement de la couche de données du client Adobe, ajouter des conditions pour le moment où la règle doit être déclenchée et envoyer le nom **de** page et le modèle **de** page d&#39;une page AEM à Adobe Analytics.
...
[https://wknd.site](https://wknd.site) est un site accessible au public créé à partir [d’un projet](https://github.com/adobe/aem-guides-wknd) open source conçu comme référence et [didacticiel](https://docs.adobe.com/content/help/en/experience-manager-learn/getting-started-wknd-tutorial-develop/overview.html) pour les implémentations AEM.

Au lieu de configurer un environnement AEM et d’installer la base de code WKND, vous pouvez utiliser le débogueur Experience Platform pour **basculer** le fichier [https://wknd.site/](https://wknd.site/) en direct vers *votre* propriété Launch. Bien sûr, vous pouvez utiliser votre propre site AEM si la couche de données du client [Adobe est déjà activée](https://docs.adobe.com/content/help/en/experience-manager-core-components/using/developing/data-layer/overview.html#installation-activation)

1. Connectez-vous à l’Experience Platform Launch et [créez une propriété](https://docs.adobe.com/content/help/en/core-services-learn/implementing-in-websites-with-launch/configure-launch/launch.html) Launch (si ce n’est déjà fait).
1. Assurez-vous qu’une [bibliothèque de lancement initiale a été créée](https://docs.adobe.com/content/help/en/launch/using/reference/publish/libraries.html#create-a-library) et promue dans un [environnement](https://docs.adobe.com/content/help/fr-FR/launch/using/reference/publish/environments.html)de lancement.
...
1. Activez **la journalisation** de la console et **verrouillez** le débogueur dans l’onglet WKND.
...
1. Développez la réponse et inspectez l’ `page` entrée. Un schéma de données doit s’afficher comme suit :
   ...
   Nous utiliserons les propriétés standard dérivées du schéma [de](https://docs.adobe.com/content/help/en/experience-manager-core-components/using/developing/data-layer/overview.html#page)page, `dc:title``xdm:language` `xdm:template` et de la couche de données pour envoyer les données de page à Adobe Analytics.

   >[!NOTE]
   >
   > Vous ne voyez pas l’objet `adobeDataLayer` javascript ? Assurez-vous que la couche de données du client [Adobe a été activée](https://docs.adobe.com/content/help/en/experience-manager-core-components/using/developing/data-layer/overview.html#installation-activation) sur votre site.

## Créer une règle chargée de page

La couche de données du client Adobe est une couche de données pilotée par **événement** . Lorsque la couche de données **Page** AEM est chargée, elle déclenche un événement `cmp:show`. Créez une règle qui sera déclenchée en fonction du `cmp:show` événement.
...
1. Cliquez sur **Ouvrir l’éditeur** dans le panneau principal et saisissez le fragment de code suivant :
   ...
   L’ `event` objet est transmis à partir de la méthode `trigger()` appelée dans le événement personnalisé. `component` est la page active dérivée de la couche de données `getState` dans le événement personnalisé. Rappelez-vous du schéma [](https://docs.adobe.com/content/help/en/experience-manager-core-components/using/developing/data-layer/overview.html#page) de page exposé par la couche de données afin de voir les différentes touches exposées hors de la zone.
...
Rappelez du début le schéma [de](https://docs.adobe.com/content/help/en/experience-manager-core-components/using/developing/data-layer/overview.html#page) page exposé par la couche de données :

Les éléments de données seront mappés sur les `@type`, `dc:title`et `xdm:template` propriétés.

### Type de ressource composant

1. Accédez à l&#39;Experience Platform Launch et à la propriété Web intégrée au site AEM.
1. Accédez à la section Eléments **de** données et cliquez sur **Créer un élément** de données.
1. Pour **Nom** , saisissez le type **de ressource de** composant.
...
1. Sous Gestion des **** bibliothèques > Suites **de** rapports, entrez les identifiants de suite de rapports à utiliser avec chaque environnement de lancement.
...
1. Définissez la première valeur du champ de formulaire sur `%Component Resource Type%`. Vous pouvez utiliser l’icône ![d’élément de](assets/collect-data-analytics/cylinder-icon.png) données Icône d’élément de données pour sélectionner l’élément de données Type **de ressource de** composant. Laissez la fonction de comparaison définie sur `Equals`.
...
1. Définissez le type d&#39; **extension** sur **Adobe Analytics** et le type **d&#39;** action sur **Définir des variables.**
...
1. Dans le panneau principal, sélectionnez un **eVar** disponible et définissez-le comme valeur du modèle **de** page d’élément de données. Utilisez l’icône Eléments de données ![Icône](assets/collect-data-analytics/cylinder-icon.png) Eléments de données pour sélectionner l’élément Modèle **de** page.

   ![Définir comme modèle de page eVar](assets/collect-data-analytics/set-evar-page-template.png)

1. Faites défiler l’écran vers le bas, sous Paramètres **** supplémentaires, définissez Nom **de** page sur l’élément de données Nom **** de page :
...
1. Cliquez sur l’icône Débogueur sur la plateforme ![Expérience - Débogueur sur l’icône](assets/collect-data-analytics/experience-cloud-debugger.png) Débogueur pour ouvrir le débogueur Experience Platform.
...
1. Faites défiler vers le bas et développez Demandes **** réseau. Vous devriez pouvoir trouver la **variable evar** définie pour le modèle **de** page :

   ![Evar et jeu de noms de page](assets/collect-data-analytics/evar-page-name-set.png)

1. Revenez au navigateur et ouvrez la console de développement. Cliquez sur le **carrousel** en haut de la page.
...
1. Observez dans la console du navigateur l’instruction console :
   ...
   Ceci est dû au fait que le carrousel déclenche un `cmp:show` événement *mais* , en raison de notre vérification du type **de ressource de** composant, aucun événement n&#39;est déclenché.
...
1. Accédez à une page d’article telle que l’Australie [](https://wknd.site/us/en/magazine/western-australia.html)occidentale. Observez que le nom de page et le type de modèle changent.
...
Consultez le didacticiel suivant pour savoir comment utiliser la couche de données client Adobe orientée événement pour [suivre les clics de composants spécifiques sur un site](track-clicked-component.md)Adobe Experience Manager.

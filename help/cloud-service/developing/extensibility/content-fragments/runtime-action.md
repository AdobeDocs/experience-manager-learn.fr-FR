---
title: AEM’extension de la console Fragment de contenu Actions Adobe I/O Runtime
...
![AEM des actions d’exécution de l’extension Fragment de contenu](./assets/runtime-action/action-runtime-flow.png){align="center"}

AEM les extensions de fragment de contenu peuvent éventuellement inclure n’importe quel nombre [Actions Adobe I/O Runtime](https://developer.adobe.com/runtime/docs/).

Les actions Adobe I/O Runtime sont des fonctions sans serveur qui peuvent être invoquées par l’extension. Les actions sont utiles pour effectuer un travail qui nécessite une interaction avec AEM ou d’autres services Web d’Adobe.
Les actions sont généralement plus utiles pour exécuter des tâches de longue durée (plus de quelques secondes) ou envoyer des requêtes HTTP à AEM ou à d’autres services Web.
...
Dans le contexte des extensions de fragments de contenu AEM, les actions sont le plus souvent utilisées pour communiquer directement avec AEM as a Cloud Service, souvent :

+ Collecte de données connexes à partir d’AEM à propos des fragments de contenu
+ Exécution d’opérations personnalisées sur des fragments de contenu
+ Création de fragments de contenu en file d’attente

Bien que l’extension de fragment de contenu AEM soit affichée dans la console Fragment de contenu, les extensions et leurs actions de prise en charge peuvent appeler n’importe quelle API HTTP d’AEM disponible, y compris les points de terminaison d’API d’AEM personnalisés.

## Appeler une action

Les actions Adobe I/O Runtime sont principalement appelées à partir de deux emplacements dans une extension de fragment de contenu AEM :
...
Les actions Adobe I/O Runtime peuvent être appelées directement à partir du code d’enregistrement de l’extension. Le cas d’utilisation le plus courant = consiste à lier une action à une [menu d’en-tête](./header-menu.md#no-modal)Bouton de qui n’utilise pas [modals](./modal.md).
...
+ [API GraphQL AEM](https://experienceleague.adobe.com/landing/experience-manager/headless/developer.html)

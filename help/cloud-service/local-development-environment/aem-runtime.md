...
1. Connectez-vous à [https://experience.adobe.com/#/downloads](https://experience.adobe.com/#/downloads) avec votre Adobe ID.
   + Notez que votre organisation d’Adobe __doit__ être configurée pour AEM en tant que Cloud Service pour télécharger l’AEM en tant que SDK Cloud Service.
1. Accédez à l&#39;AEM __en tant que Cloud Service__ onglet
1. Trier par __Date de publication__ dans __ordre__ Descendant
1. Cliquez sur la dernière ligne de résultat __AEM SDK__
1. Examinez et acceptez le CLUF et appuyez sur le bouton __Télécharger__.
...
## Configuration du service local AEM Author{#set-up-local-aem-author-service}
...
1. Copiez le fichier __JAR de démarrage rapide__ dans `~/aem-sdk/author` et renommez-le en `aem-author-p4502.jar`.
1. Début du service d’auteur AEM local en exécutant les éléments suivants à partir de la ligne de commande :
   ...
   Vous *ne pouvez pas* début l&#39;AEM en tant que Jar de démarrage rapide Cloud Service [en cliquant sur le doublon ](#troubleshooting-double-click).
1. Accédez au service d’auteur AEM local à l’adresse [http://localhost:4502](http://localhost:4502) dans un navigateur Web.
...
Dans un véritable environnement Cloud Service, le contenu est distribué du service d’auteur au service de publication à l’aide de [Sling Content Distribution](https://sling.apache.org/documentation/bundles/content-distribution.html) et du canal d’Adobe. Le [pipeline d&#39;Adobe](https://experienceleague.adobe.com/docs/experience-manager-cloud-service/core-concepts/architecture.html?lang=en#content-distribution) est un microservice isolé disponible uniquement dans l&#39;environnement de nuage.
...
1. Vous pouvez maintenant apporter des modifications au contenu du service Auteur et les publier dans le service Publier.
...
+ Le port __4502__ est utilisé pour le __service local AEM Author__
+ Le port __4503__ est utilisé pour le __service local de publication AEM__
...
## Le démarrage du fichier JAR de démarrage rapide à partir de la ligne de commande annule immédiatement{#troubleshooting-java-8}

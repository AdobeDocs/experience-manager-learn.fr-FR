...
1. Si Homebrew n’est pas installé, installez Homebrew.
   + [Installer Homebrew sur macOS](https://brew.sh/)
      + Homebrew sur macOS requiert [Xcode](https://apps.apple.com/us/app/xcode/id497799835) ou des outils [de ligne de](https://developer.apple.com/download/more/)commande, installable via la commande :
...
Si vous utilisez Homebrew, suivez les instructions __Install using Homebrew__ (Installation à l’aide d’Homebrew)dans les sections ci-dessous. Si vous __n’utilisez pas__ Homebrew, installez les outils à l’aide des liens propres au système d’exploitation.

## Installer Git

[Git](https://git-scm.com/) est le système de gestion du contrôle de code source utilisé par [Adobe Cloud Manager](https://docs.adobe.com/content/help/en/experience-manager-cloud-manager/using/requirements/source-code-repository.html)et est donc requis pour le développement.
...
[Node.js](https://nodejs.org) est un environnement d’exécution JavaScript utilisé pour travailler avec les actifs frontaux d’un sous-projet __ui.frontend__ d’un projet AEM. Node.js est distribué avec [npm](https://www.npmjs.com/), est le gestionnaire de package de Node.js par défaut, utilisé pour gérer les dépendances JavaScript.
...
>
>Reportez-vous à cet exemple [AEM Project Reactor pom.xml](https://github.com/adobe/aem-guides-wknd/blob/9ac94f3f40c978a53ec88fae79fbc17dd2db72f2/pom.xml#L117-L118) pour connaître l’emplacement des versions de création Node.js et npm.

## Installation de Maven

Apache Maven est l&#39;outil de ligne de commande Java open-source utilisé pour construire AEM projets générés à partir de l&#39;archétype AEM Project Maven. Tous les principaux IDE ([IntelliJ IDEA](https://www.jetbrains.com/idea/), code [](https://code.visualstudio.com/)Visual Studio, [Eclipse](https://www.eclipse.org/), etc.) sont pris en charge par Maven.
...
## Configuration de l&#39;interface de ligne de commande d&#39;E/S d&#39;Adobe{#aio-cli}

L&#39;interface de ligne de commande des E/S [Adobes](https://github.com/adobe/aio-cli), ou `aio`fournit un accès en ligne de commande à divers services d&#39;Adobe, y compris [Cloud Manager](https://github.com/adobe/aio-cli-plugin-cloudmanager) et [Asset Compute](https://github.com/adobe/aio-cli-plugin-asset-compute). L&#39;interface de ligne de commande des E/S Adobe joue un rôle essentiel dans le développement de l&#39;AEM en tant que Cloud Service, car elle permet aux développeurs de :
...
### Installation de l&#39;interface de ligne de commande d&#39;E/S de l&#39;Adobe

1. Assurez-vous que [Node.js est installé](#node-js) car l&#39;interface de ligne de commande des E/S Adobe est un module npm.
   + Exécuter `node --version` pour confirmer
1. Exécuter `npm install -g @adobe/aio-cli` pour installer le module `aio` npm globalement

### Configuration du module externe CLI Cloud Manager d’E/S Adobe{#aio-cloud-manager}

Le module externe Adobe E/S Cloud Manager permet à l’interface de ligne de commande d’Ai d’interagir avec Adobe Cloud Manager via la `aio cloudmanager` commande.

1. Exécutez `aio plugins:install @adobe/aio-cli-plugin-cloudmanager` l’installation du module externe [](https://github.com/adobe/aio-cli-plugin-cloudmanager)aio Cloud Manager.

### Configuration du module externe CLI Asset Compute d&#39;E/S Adobe{#aio-asset-compute}

Le module complémentaire Adobe E/S Cloud Manager permet à l’interface de ligne de commande d’Ai de générer et d’exécuter des agents Asset Compute via la `aio asset-compute` commande.

1. Exécutez `aio plugins:install @adobe/aio-cli-plugin-cloudmanager` pour installer le module externe [](https://github.com/adobe/aio-cli-plugin-asset-compute)aio Asset Compute.

### Configuration de l&#39;authentification CLI d&#39;E/S Adobe

Pour que l’interface de ligne de commande des E/S d’Adobe communique avec Cloud Manager, une intégration de Cloud Manager doit être créée dans la console d’E/S d’Adobe et les informations d’identification doivent être obtenues pour s’authentifier.
...
1. Créer ou ouvrir un programme d&#39;E/S [Adobe existant](https://www.adobe.io/apis/experienceplatform/console/docs.html#!AdobeDocs/adobeio-console/master/projects.md)
   + Les programmes de la console d&#39;E/S d&#39;Adobe sont simplement des regroupements organisationnels d&#39;intégrations, de créer ou d&#39;utiliser et des programmes existants en fonction de la façon dont vous souhaitez gérer vos intégrations.
   + Si vous créez un projet, sélectionnez &quot;Projet vide&quot; si vous y êtes invité (par rapport à Créer à partir d’un modèle).
   + Les programmes de la console d&#39;E/S d&#39;Adobe sont des concepts différents pour les programmes de Cloud Manager
1. Créer une nouvelle intégration de l’API Cloud Manager avec le profil &quot;Développeur - Cloud Service&quot;
1. Obtenez les informations d’identification du compte de service (JWT) nécessaires pour renseigner le fichier [config.json de l’interface de ligne de commande d’Adobe.](https://github.com/adobe/aio-cli-plugin-cloudmanager#authentication)
1. Charger le `config.json` fichier dans l&#39;interface de ligne de commande d&#39;E/S de l&#39;Adobe
   + `$ aio config:set jwt-auth PATH_TO_CONFIG_JSON_FILE --file --json`
1. Charger le `private.key` fichier dans l&#39;interface de ligne de commande d&#39;E/S de l&#39;Adobe
   + `$ aio config:set jwt-auth.jwt_private_key PATH_TO_PRIVATE_KEY_FILE --file`

Commencez à [exécuter des commandes](https://github.com/adobe/aio-cli-plugin-cloudmanager#commands) pour Cloud Manager via l’interface de ligne de commande des E/S Adobe.
...
Visual Studio Code est le choix idéal pour les développeurs frontaux qui créent principalement du code frontal ; JavaScript, CSS et HTML. Bien que VS Code dispose de la prise en charge de Java par [les extensions](https://code.visualstudio.com/docs/java/java-tutorial), il peut ne pas disposer de certaines fonctionnalités avancées fournies par des fonctionnalités plus spécifiques à Java.
...
__[Eclipse IDE](https://www.eclipse.org/ide/)__ est un IDE très utilisé pour le développement de Java et prend en charge le module externe Outils __[de développement](https://eclipse.adobe.com/aem/dev-tools/)__ AEM fourni par Adobe. Il fournit une interface utilisateur graphique intégrée pour la création et la synchronisation de contenu JCR avec une instance AEM locale.

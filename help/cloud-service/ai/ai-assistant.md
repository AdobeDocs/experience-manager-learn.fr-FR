---
title: Assistant IA AEM
description: Simplifiez AEM avec l’assistant d’IA. Découvrez les fonctionnalités, les bonnes pratiques et corrigez les problèmes tout en améliorant la productivité grâce à une prise en charge optimisée par l’IA.
version: Experience Manager as a Cloud Service
role: Leader, User
level: Beginner
doc-type: Feature Video
duration: 205
last-substantial-update: 2025-12-04T00:00:00Z
jira: KT-19319
source-git-commit: 96719e9f53324469927eee83840356833b5605a0
workflow-type: tm+mt
source-wordcount: '91'
ht-degree: 6%

---


# Assistant IA AEM

Découvrez comment l’assistant AEM AI [&#128279;](https://experienceleague.adobe.com/fr/docs/experience-manager-cloud-service/content/ai-in-aem/ai-assistant/ai-assistant-in-aem#) simplifie vos workflows Adobe Experience Manager. Qu’il s’agisse de trouver des fonctionnalités, d’en savoir plus sur les bonnes pratiques, de résoudre les problèmes ou même de créer des tickets d’assistance, AEM est votre partenaire intelligent. Stimulez votre productivité grâce aux conseils, informations et assistance optimisés par l’IA, le tout en un seul endroit.

>[!VIDEO](https://video.tv.adobe.com/v/3475360/?captions=fre_fr&learn=on&enablevpops)

*Pour accéder à l’assistant AEM AI, vérifiez que vous avez configuré l’accès dans votre environnement AEM en suivant le [guide de configuration](./setup.md#aem-ai-assistant).*
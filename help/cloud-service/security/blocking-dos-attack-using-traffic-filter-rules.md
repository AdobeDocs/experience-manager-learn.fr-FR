---
title: Blocage des attaques par déni de service, DDoS et sophistiquées à l’aide de règles de filtrage du trafic
description: Découvrez comment bloquer les attaques par déni de service, par déni de service et les attaques sophistiquées à l’aide des règles de filtrage du trafic dans AEM as a Cloud Service.
...
# Blocage des attaques par déni de service, DDoS et sophistiquées à l’aide de règles de filtrage du trafic

Découvrez comment bloquer les attaques par déni de service (DoS), par déni de service distribué (DDoS) et les attaques sophistiquées à l’aide des **règles de filtrage du trafic** dans le réseau CDN géré par AEM as a Cloud Service (AEMCS).
...
Cet article présente les protections par défaut de votre site web AEM et explique comment étendre ces protections grâce à la configuration client. Il décrit également comment analyser les modèles de trafic et configurer des règles de filtrage de trafic standard pour bloquer ces attaques.
...
Ces protections intégrées doivent être considérées comme une référence pour la capacité d’une organisation à minimiser l’impact d’une attaque DDoS sur les performances. Comme chaque site web présente des caractéristiques de performances différentes et peut voir une dégradation des performances avant que la limite de débit définie par Adobe ne soit atteinte, il est recommandé d’étendre les protections par défaut via _configuration du client_.
...
- Utilisez le module complémentaire **Protection WAF-DDoS** ou **Sécurité renforcée** et implémentez les [règles de filtrage du trafic WAF recommandées par Adobe](./traffic-filter-and-waf-rules/use-cases/using-waf-rules.md) pour vous défendre contre les attaques sophistiquées, y compris celles qui utilisent un protocole avancé ou des techniques basées sur la payload.
- Augmentez la couverture du cache en configurant [transformations de requête](./traffic-filter-and-waf-rules/how-to/request-transformation.md) pour ignorer les paramètres de requête inutiles.
        ...
        <div class="card" style="height: 100%; display: flex; flex-direction: column; height: 100%;">
            <div class="card-image">
                <figure class="image x-is-16by9">
                    <a href="./traffic-filter-and-waf-rules/setup.md" title="Configuration des règles de filtrage du trafic, y compris les règles WAF" target="_self" rel="referrer">
                        <img class="is-bordered-r-small" src="./traffic-filter-and-waf-rules/assets/setup/rules-setup.png" alt="Configuration des règles de filtrage du trafic, y compris les règles WAF"
            ...
            <div class="card-content is-padded-small" style="display: flex; flex-direction: column; flex-grow: 1; justify-content: space-between;">
                <div class="top-card-content">
                    <p class="headline is-size-6 has-text-weight-bold">
                        <a href="./traffic-filter-and-waf-rules/setup.md" target="_self" rel="referrer" title="Configuration des règles de filtrage du trafic, y compris les règles WAF">Configuration de règles de filtrage du trafic, y compris des règles WAF</a>
                    </p>

---
title: Configurer AEM Assets Content Hub
description: Découvrez comment configurer Adobe Experience Manager Assets Content Hub sur AEM as a Cloud Service.
version: Experience Manager as a Cloud Service
feature: Configuration, Asset Management
topic: Content Supply Chain, Administration
role: Admin
level: Beginner
doc-type: Feature Video
duration: 435
last-substantial-update: 2024-01-10T00:00:00Z
jira: KT-15989
thumbnail: KT-15989.jpeg
exl-id: 83ca35c5-1db4-4297-afc7-67b6e7d2f085
source-git-commit: 998a691bdc001032880d2d8e05f0bd18554c2848
workflow-type: tm+mt
source-wordcount: '108'
ht-degree: 56%

---

# Configurer AEM Assets Content Hub

Découvrez comment configurer Experience Manager Assets Content Hub à l’aide du module complémentaire Ressources et du programme Adobe Cloud Manager. Découvrez également comment configurer l’accès pour les utilisateurs et utilisatrices qui utilisent des profils de produit.

>[!VIDEO](https://video.tv.adobe.com/v/3472918/?learn=on)


## Configurer les API ouvertes Dynamic Media AEM Assets

Si vous disposez d’une licence OpenAPI Dynamic Media AEM Assets, vous pouvez les utiliser pour générer des rendus dynamiques dans Content Hub, tels que les recadrages intelligents. Avant que ces rendus OpenAPI Dynamic Media puissent apparaître dans Content Hub, une configuration initiale est requise.

>[!SLIDE](dm-open-api-setup)

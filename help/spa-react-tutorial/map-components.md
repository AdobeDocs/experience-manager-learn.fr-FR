...
Découvrez comment mapper des composants Réagir aux composants Adobe Experience Manager (AEM) avec l’AEM SDK JS de l’éditeur SPA. Le mappage de composants permet aux utilisateurs d’effectuer des mises à jour dynamiques sur SPA composants dans AEM Éditeur de SPA, comme pour la création d’ traditionnelle.
...
Vous pouvez toujours vue le code terminé sur [GitHub](https://github.com/adobe/aem-guides-wknd-spa/tree/React/map-components-solution) ou vérifier le code localement en passant à la branche `React/map-components-solution`.
...
1. Ouvrez un nouveau terminal et accédez au dossier `ui.frontend` dans le projet. Exécutez `npm install`, puis `npm start` pour début du **webpack-dev-server** :
...
2. Vous devriez voir une nouvelle fenêtre de navigateur ouverte sur [http://localhost:3000/content/wknd-spa-react/us/en/home.html](http://localhost:3000/content/wknd-spa-react/us/en/home.html).
...
4. La première zone que nous allons inspecter est la `class Text` à ~ligne 40 :
   ...
   `Text` est un composant React standard. Le composant utilise `this.props.richText` pour déterminer si le contenu à rendre sera du texte enrichi ou du texte brut. Le &quot;contenu&quot; réel utilisé provient de `this.props.text`. Pour éviter une attaque XSS potentielle, le texte enrichi est échappé par `DOMPurify` avant d’utiliser [dangereusementSetInnerHTML](https://reactjs.org/docs/dom-elements.html#dangerouslysetinnerhtml) pour générer le contenu. Rappelez les propriétés `richText` et `text` du modèle JSON plus tôt dans l’exercice.

5. Jetez ensuite un coup d&#39;oeil au `TextEditConfig` à la ligne 29 :
...
6. Enfin, jetez un coup d&#39;oeil à l&#39;appel `MapTo` à la ligne 62 :

   ```js
   export default MapTo('wknd-spa-react/components/text')(Text, TextEditConfig);
   ```

   `MapTo` est fourni par AEM Editor SPA JS SDK (`@adobe/aem-react-editable-components`). Le chemin `wknd-spa-react/components/text` représente le `sling:resourceType` du composant AEM. Ce chemin est mis en correspondance avec le `:type` exposé par le modèle JSON observé précédemment. `MapTo` prend soin d’analyser la réponse du modèle JSON et de transmettre les valeurs correctes  `props` au composant SPA.

   Vous trouverez l&#39;AEM de définition de composant `Text` à l&#39;adresse `ui.apps/src/main/content/jcr_root/apps/wknd-spa-react/components/text`.

7. Testez en modifiant le fichier `mock.model.json` à `ui.frontend/public/mock-content/mock.model.json`. À ~line 62, mettez à jour la première valeur `Text` pour utiliser des balises **`H1`** et **`u`** :
...
8. Inspect `Text.scss` à `ui.frontend/src/components/Text/Text.scss`.
   ...
   Ce fichier a été ajouté par la base de code de démarrage pour ce chapitre et utilise la fonction [Sass](https://sass-lang.com/) ajoutée dans le chapitre précédent. Notez les variables référencées à partir de `ui.frontend/src/styles/_variables.scss`.

## Création du composant Image

Ensuite, créez un composant `Image` Réagir qui est mappé au composant AEM [Image](https://docs.adobe.com/content/help/fr-FR/experience-manager-core-components/using/components/image.html). Le composant `Image` est un autre exemple de composant **content**.
...
2. Revenez à votre IDE et ouvrez le `mock.model.json` à `ui.frontend/public/mock-content/mock.model.json`. Comme il s&#39;agit d&#39;un composant net-new pour notre projet, nous devons nous moquer de l&#39;image JSON.
   ...
   À ~line 70, ajoutez une entrée JSON pour le modèle `image` (n&#39;oubliez pas la virgule de fin `,` après la seconde `text_23828680`) et mettez à jour le tableau `:itemsOrder`.
   ...
   Le projet comprend un exemple d&#39;image à `/mock-content/adobestock-140634652.jpeg` qui sera utilisé avec le **webpack-dev-server**.

   Vous pouvez vue le [mock.model.json complet ici](https://github.com/adobe/aem-guides-wknd-spa/blob/React/map-components-solution/ui.frontend/public/mock-content/mock.model.json).

### Mise en oeuvre du composant Image

1. Créez ensuite un nouveau dossier nommé `Image` sous `ui.frontend/src/components`.
2. Sous le dossier `Image`, créez un nouveau fichier nommé `Image.js`.
...
4. Ajoutez ensuite le `ImageEditConfig` pour déterminer à quel moment afficher l’espace réservé dans AEM :
...
6. Ajoutez le code `MapTo` pour mapper le composant Réagir au composant AEM :
...
8. Dans `Image.js`, ajoutez une référence au fichier en haut sous les instructions `import` :
   ...
   Vous pouvez vue le fichier [Image.js terminé ici](https://github.com/adobe/aem-guides-wknd-spa/blob/React/map-components-solution/ui.frontend/src/components/Image/Image.js).
...
10. Si ce n’est pas déjà fait, début le **webpack-dev-server**. Accédez à [http://localhost:3000](http://localhost:3000) et vous devriez voir le rendu de l’image :

   ![Image ajoutée à la maquette](./assets/map-components/image-added-mock.png)

   >[!NOTE]
   >
   > **Défi** : Mettez en oeuvre une nouvelle méthode dans  `Image.js` pour afficher la valeur de  `this.props.title` sous-image en tant que légende.
...
2. Dans l’écran Début de l’AEM, accédez à **Outils** > **Modèles** > **[WKND SPA Réagir](http://localhost:4502/libs/wcm/core/content/sites/templates.html/conf/wknd-spa-react)**.
...
4. Sous **Composants autorisés** > **Réaction de la SPA WKND - Contenu** > vérifiez le composant **Image** :

   ![Composant d’image sélectionné](./assets/map-components/check-image-component.png)

   Sous **Composants par défaut** > **Ajouter le mappage** et sélectionnez le composant **Image - WKND SPA Réaction - Contenu** :
...
2. Revenir à [http://localhost:4502/editor.html/content/wknd-spa-react/us/en/home.html](http://localhost:4502/editor.html/content/wknd-spa-react/us/en/home.html). Ajoutez d’autres composants **Image** et essayez de les redimensionner à l’aide de l’option **Disposition** :

   ![Redimensionner l’image en mode Mise en page](./assets/map-components/responsive-grid-layout-change.gif)

3. Ouvrez de nouveau le modèle JSON [http://localhost:4502/content/wknd-spa-react/us/en.model.json](http://localhost:4502/content/wknd-spa-react/us/en.model.json) et observez `columnClassNames` dans le fichier JSON :
...
4. Revenez à l&#39;IDE et dans le module `ui.apps`, il y a une bibliothèque côté client définie à `ui.apps/src/main/content/jcr_root/apps/wknd-spa-react/clientlibs/clientlib-grid`. Ouvrez le fichier `less/grid.less`.
...
5. Vous devez pouvoir utiliser les fonctionnalités réactives et les stratégies de texte enrichi mises à jour du composant `Text` pour créer une vue du type suivant :
...
[Navigation et Routage](navigation-routing.md)  : découvrez comment plusieurs vues de la SPA peuvent être prises en charge en mappant des pages AEM avec le SDK de l’éditeur SPA. La navigation dynamique est mise en oeuvre à l&#39;aide du Routeur de réaction et ajoutée à un composant d&#39;en-tête existant.
...
4. Répétez les étapes pour importer le contenu, mais sélectionnez le dossier **policies** situé à `/conf/wknd-spa-react/settings/wcm/templates/policies`.

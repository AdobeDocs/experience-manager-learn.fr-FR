...
Guide de configuration d&#39;un développement local pour Adobe Experience Manager, AEM. Couvre les principaux sujets de l&#39;installation locale, Apache Maven, les environnements de développement intégré et le débogage/dépannage. Le développement avec **[!DNL Eclipse IDE],[!DNL CRXDE Lite],[!DNL Visual Studio Code]et[!DNL IntelliJ]** est discuté.
...
Lorsque nous faisons référence à une instance d’AEM locale, nous parlons d’une copie de Adobe Experience Manager exécutée sur la machine personnelle d’un développeur. ***Tout*** développement AEM doit début en écrivant et en exécutant du code par rapport à une instance AEM locale.

Si vous êtes novice en AEM, deux modes d’exécution de base peuvent être installés : ***Auteur*** et ***publication***. Le mode ***d’exécution*** Auteur [](https://helpx.adobe.com/fr/experience-manager/6-5/sites/deploying/using/configure-runmodes.html) est l’environnement que les spécialistes du marketing numérique utiliseront pour créer et gérer du contenu. Lorsque vous développez **la plupart** du temps, vous déployez du code sur une instance d’auteur. Vous pouvez ainsi créer de nouvelles pages, ajouter et configurer des composants. aem sites est un CMS de création WYSIWYG et la plupart des CSS et JavaScript peuvent donc être testés par rapport à une instance de création.

Il s’agit également d’un code de test *critique* par rapport à une instance de ***publication*** locale. L’instance de ***publication*** est l’environnement AEM avec lequel les visiteurs de votre site Web interagiront. Bien que l’instance de ***publication*** soit la même pile technologique que l’instance d’ ***auteur*** , il existe certaines différences majeures avec les configurations et les autorisations. Le code doit *toujours* être testé par rapport à une instance de ***publication*** locale avant d’être promu à des environnements de niveau supérieur.
...
2. Récupérez une copie de l&#39; [AEM JAR QuickStart et une [!DNL license.properties]](https://helpx.adobe.com/experience-manager/6-5/sites/deploying/using/deploy.html#GettingtheSoftware).
...
4. Renommez le [!DNL QuickStart] fichier JAR en ***aem-author-p4502.jar*** et placez-le sous le `/author` répertoire. ajoutez le ***[!DNL license.properties]*** fichier sous le `/author` répertoire.
5. Effectuez une copie du fichier [!DNL QuickStart] JAR, renommez-le ***en aem-publish-p4503.jar*** et placez-le sous le `/publish` répertoire. ajoutez une copie du ***[!DNL license.properties]*** fichier situé sous le `/publish` répertoire.
...
6. Doublon-cliquez sur le fichier ***aem-author-p4502.jar*** pour installer l’instance **Auteur** . L’instance d’auteur est ainsi début et s’exécute sur le port **4502** sur l’ordinateur local.

   Doublon-cliquez sur le fichier ***aem-publish-p4503.jar*** pour installer l’instance **de publication** . L’instance de publication est alors début, s’exécutant sur le port **4503** sur l’ordinateur local.

   >[!NOTE]
   >
   >Selon le matériel de votre machine de développement, il peut être difficile d’exécuter simultanément une instance **Auteur et Publication** . Il est rare que vous deviez exécuter les deux simultanément sur une configuration locale.

   Pour plus d’informations, voir [Déploiement et gestion d’une instance](https://helpx.adobe.com/fr/experience-manager/6-5/sites/deploying/using/deploy.html)AEM.
...
Tous les projets AEM doivent être construits à partir de la dernière version de **[!DNL AEM Project Archetype]**: [https://github.com/Adobe-Marketing-Cloud/aem-project-archetype](https://github.com/Adobe-Marketing-Cloud/aem-project-archetype). Le [!DNL AEM Project Archetype] créera un bootstrap d&#39;un projet AEM avec un exemple de code et de contenu. Le [!DNL AEM Project Archetype] comprend également **[!DNL AEM WCM Core Components]** configuré pour être utilisé sur votre projet.
...
4. ajoutez le **[!DNL adobe-public]** profil dans votre fichier [!DNL Maven] settings.xml [afin d’ajouter automatiquement](https://maven.apache.org/settings.html) **[!DNL repo.adobe.com]** au processus de création de l’expert.

5. Créez un fichier nommé `settings.xml` à `~/.m2/settings.xml` s&#39;il n&#39;existe pas déjà.

6. ajoutez le **[!DNL adobe-public]** profil dans le `settings.xml` fichier en fonction [des instructions fournies ici](https://repo.adobe.com/).

   A sample `settings.xml` is listed below. *Notez que la convention d’affectation de nom`settings.xml`et l’emplacement sous le`.m2`répertoire de l’utilisateur sont importants.*
...
L&#39; **[[!DNL Eclipse] IDE](https://www.eclipse.org/ide/)** est l&#39;un des IDE les plus populaires pour le développement Java, en grande partie parce qu&#39;il est open source et ***gratuit***! adobe fournit un module externe, **[[ !DNL AEM Developer Tools]](https://eclipse.adobe.com/aem/dev-tools/)**, pour [!DNL Eclipse] permettre un développement plus facile avec une interface graphique graphique agréable pour synchroniser le code avec une instance AEM locale. L&#39; [!DNL Eclipse] IDE est recommandé pour les développeurs qui viennent d&#39;AEM en grande partie grâce à la prise en charge de l&#39;interface utilisateur graphique par [!DNL AEM Developer Tools].

#### Installation et configuration

1. Téléchargez et installez [!DNL Eclipse] IDE pour [!DNL Java EE Developers]: [https://www.eclipse.org](https://www.eclipse.org/)
1. Suivez les instructions pour installer le [!DNL AEM Developer Tools] module externe : [https://eclipse.adobe.com/aem/dev-tools/](https://eclipse.adobe.com/aem/dev-tools/)
...
L&#39;IDEA **[IntelliJ](https://www.jetbrains.com/idea/)** est un IDE puissant pour le développement professionnel de Java. [!DNL IntelliJ IDEA] est disponible en deux versions, une édition ***gratuite*** et une [!DNL Community] [!DNL Ultimate] version commerciale (payante). La version gratuite [!DNL Community] de [!DNL IntellIJ IDEA] est suffisante pour un développement plus AEM, mais le [!DNL Ultimate] développement de ses capacités est [élargi](https://www.jetbrains.com/idea/download).
...
1. Installation [!DNL Repo] (outil de ligne de commande) : [https://github.com/Adobe-Marketing-Cloud/tools/tree/master/repo](https://github.com/Adobe-Marketing-Cloud/tools/tree/master/repo#installation)
...
* **[aem Sync](https://marketplace.visualstudio.com/items?itemName=Yinkai15.aemsync)** - Extension prise en charge par la communauté* pour le code Visual Studio
...
[CRXDE Lite](https://helpx.adobe.com/experience-manager/6-4/sites/developing/using/developing-with-crxde-lite.html) est une vue du référentiel AEM basée sur un navigateur. [!DNL CRXDE Lite] est intégrée dans AEM et permet au développeur d’effectuer des tâches de développement standard telles que la modification de fichiers, la définition de composants, de boîtes de dialogue et de modèles. [!DNL CRXDE Lite] n’est ***pas*** conçu pour être un environnement de développement complet, mais est très efficace comme outil de débogage. [!DNL CRXDE Lite] s’avère utile lors de l’extension ou simplement de la compréhension du code de produit en dehors de votre base de code. [!DNL CRXDE Lite] fournit une vue puissante du référentiel et un moyen de tester et de gérer efficacement les autorisations.
...
1. Si vous effectuez des mises à jour incrémentielles de fichiers à l’aide d’un outil tel que [!DNL Repo] ou [!DNL AEM Developer Tools], **vérifiez[!DNL CRXDE Lite]** que le fichier a été déplacé vers l’instance d’AEM locale et que le contenu du fichier est mis à jour : [http://localhost:4502/crx/de/index.jsp](http://localhost:4502/crx/de/index.jsp)
1. **Vérifiez que le lot est téléchargé** si des problèmes liés au code Java apparaissent dans un lot OSGi. Ouvrez la console [!UICONTROL Web]Adobe Experience Manager : [http://localhost:4502/system/console/bundles](http://localhost:4502/system/console/bundles) et recherchez votre lot. Assurez-vous que le lot a un état **[!UICONTROL Principal]** . Voir ci-dessous pour plus d’informations sur le dépannage d’un lot dans un état **[!UICONTROL Installé]** .

#### Vérification des journaux

aem est une plate-forme de discussion et consigne de nombreuses informations utiles dans le fichier **error.log**. Le fichier **error.log** se trouve là où AEM a été installé : &lt; `aem-installation-folder>/crx-quickstart/logs/error.log`.
...
Par défaut, **error.log** est configuré pour enregistrer les *[!DNL INFO]* instructions. Si vous souhaitez modifier le niveau de journal, vous pouvez le faire en accédant à Prise en charge [!UICONTROL des]journaux : [http://localhost:4502/system/console/slinglog](http://localhost:4502/system/console/slinglog). Vous pouvez également trouver que le **fichier error.log** est trop bavard. Vous pouvez utiliser la prise en charge [!UICONTROL des] journaux pour configurer des instructions de journal pour un package Java spécifié uniquement. Il s’agit de la meilleure pratique pour les projets, afin de séparer facilement les problèmes de code personnalisé des problèmes de plateforme AEM prêtes à l’emploi.
...
Tous les lots (à l’exception des fragments) doivent être à l’état **[!UICONTROL Principal]** . Si votre lot de code s’affiche à l’état [!UICONTROL Installé] , un problème doit être résolu. La plupart du temps, il s&#39;agit d&#39;un problème de dépendance :

![Erreur de regroupement dans AEM](assets/set-up-a-local-aem-development-environment/bundle-error.png)

Dans la capture d&#39;écran ci-dessus, l&#39; [!DNL WKND Core bundle] état est [!UICONTROL Installé] . En effet, le lot attend une version différente de `com.adobe.cq.wcm.core.components.models` celle disponible sur l’instance AEM.

L’outil de recherche [!UICONTROL de]dépendance est un outil utile : [http://localhost:4502/system/console/depfinder](http://localhost:4502/system/console/depfinder). ajoutez le nom du package Java pour vérifier quelle version est disponible sur l’instance AEM :

![Composants principaux](assets/set-up-a-local-aem-development-environment/core-components.png)

En suivant l’exemple ci-dessus, nous constatons que la version installée sur l’instance AEM est **12.2** par rapport à **12.6** que le lot attendait. A partir de là, vous pouvez travailler à l&#39;envers et voir si les [!DNL Maven] dépendances d&#39;AEM correspondent aux [!DNL Maven] dépendances du projet AEM. Dans l’exemple ci-dessus, [!DNL Core Components] v2.2.0 **est installé sur l’instance AEM mais le lot de code a été créé avec une dépendance sur** v2.2.2 ****, d’où la raison du problème de dépendance.

#### Vérification de l’enregistrement des modèles Sling {#osgi-component-sling-models}

aem composants doivent toujours être sauvegardés par un [!DNL Sling Model] pour encapsuler toute logique métier et s’assurer que le script de rendu HTML reste propre. Si vous rencontrez des problèmes où le modèle Sling est introuvable, il peut s’avérer utile de vérifier le modèle [!DNL Sling Models] à partir de la console : [http://localhost:4502/system/console/status-slingmodels](http://localhost:4502/system/console/status-slingmodels). Cela vous dira si votre modèle Sling a été enregistré et à quel type de ressource (le chemin du composant) il est lié.

![État du modèle Sling](assets/set-up-a-local-aem-development-environment/sling-model-status.png)

Affiche l&#39;enregistrement d&#39;un [!DNL Sling Model], `BylineImpl` qui est lié à un type de ressource de composant de `wknd/components/content/byline`.
...
>[!NOTE]
>
>Si vous devez invalider constamment le cache à l&#39;aide de l&#39;outil [!UICONTROL Reconstruire les bibliothèques] clientes, il peut s&#39;avérer utile de reconstruire une seule fois toutes les bibliothèques clientes. Cela peut prendre environ 15 minutes, mais élimine généralement les problèmes de mise en cache à l’avenir.
